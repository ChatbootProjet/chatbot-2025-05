/**
 * Enhanced AI Chatbot - Main Frontend Script
 * Provides interactive functionality for the chatbot interface
 * With improved message handling, scroll management, and markdown support
 */
...
let scrollTimeout = null; // For scroll event throttling
let previousLastMessageId = null; // To track new messages
let learningChart = null; // For statistics chart
let messagesObserver = null; // Intersection observer for messages
...
function initializeChat() {
    ...
    setTimeout(() => {
        scrollToBottom(false); // No animation on init
    }, 100);
}

// Setup intersection observer for messages
function setupIntersectionObserver() {
    // Create observer to monitor when messages enter/exit the viewport
...
function setupEventListeners() {
    ...
    statsButton.addEventListener('click', () => {
        ...
        // Initialize chart when modal is opened
    ...
    document.addEventListener('click', (e) => {
        // Don't focus if clicking inside a modal or on a button
...
function sendMessage() {
    ...
    if (messageHistory.length === 0 || messageHistory[0] !== messageText) {
...
function addMessage(text, sender) {
    ...
    messageElement.dataset.id = `msg_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    messageElement.dataset.read = isAutoScrollEnabled ? 'true' : 'false';
    ...
    if (sender === 'bot') {
        ...
        try {
            ...
            const formattedText = text.replace(/\n(?!\n)/g, '\n\n'); // Add extra line breaks for single line breaks
            ...
            contentElement.querySelectorAll('pre code').forEach(codeBlock => {
                ...
                if (langMatch && langMatch[1]) {
                    ...
                    pre.dataset.language = langMatch[1];
                }
            });
        } catch (error) {
    ...
    // Start observing the new message
    messagesObserver.observe(messageElement);
...
function scrollToBottom(animate = true) {
    ...
    // If no animation requested or we're already close to the bottom, just jump
    if (!animate || Math.abs(scrollDistance) < 50) {
...
function initializeStatisticsChart() {
    const chartCanvas = document.getElementById('learning-chart');
    ...
    .then(response => response.json())
    .then(data => {
        if (!data || !data.stats_over_time) {
            return;
        }
        
        const timeLabels = data.stats_over_time.map(item => item.date);
        const learnedData = data.stats_over_time.map(item => item.learned);
        const conversationsData = data.stats_over_time.map(item => item.conversations);
...
function updateStatistics(stats) {
    ...
    if (learningChart && stats.stats_over_time) {
        const timeLabels = stats.stats_over_time.map(item => item.date);
        const learnedData = stats.stats_over_time.map(item => item.learned);
        const conversationsData = stats.stats_over_time.map(item => item.conversations);
        
        learningChart.data.labels = timeLabels;
        learningChart.data.datasets[0].data = learnedData;
        learningChart.data.datasets[1].data = conversationsData;
...
function renderConversationList(conversations) {
    ...
    conversations.forEach(conv => {
        ...
        // Format preview text by truncating if needed
        const previewText = conv.preview || 'No messages';
        const truncatedPreview = previewText.length > 30 
...
function detectLanguage(text) {
    ...
    // Count Arabic vs Latin characters for more accurate detection
    if (hasArabic) {
        // If only Arabic, or predominantly Arabic
        if (!hasLatin) return 'rtl';
        
        // Count characters to determine dominant script
