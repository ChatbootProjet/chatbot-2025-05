...
echo.
echo Adding firewall rules for AI Chatbot...
...
netsh advfirewall firewall add rule name="AI Chatbot - Inbound" dir=in action=allow protocol=TCP localport=5000

REM Add outbound rule for port 5000 (optional but recommended)
echo Adding outbound rule for port 5000...
netsh advfirewall firewall add rule name="AI Chatbot - Outbound" dir=out action=allow protocol=TCP localport=5000
...
echo - Local machine: http://localhost:5000
echo - Network devices: http://[YOUR-IP]:5000
echo.
